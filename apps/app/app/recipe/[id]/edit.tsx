...
import type { Category, RecipeDifficulty } from '@recetario/shared'
...
export default function EditRecipeScreen() {
  ...
  useEffect(() => {
    ...
    setPrepTimeMin(form.prepTimeMin)
    setCookTimeMin(form.cookTimeMin)
    setDifficulty(form.difficulty)
  }, [recipe])

  const mutation = useMutation({
    mutationFn: (data: Parameters<typeof api.recipes.update>[1]) => api.recipes.update(id, data),
    onSuccess: () => {
  ...
  function handleSubmit() {
    ...
    setErrors({})
    mutation.mutate({ ...payload, visibility } as Parameters<typeof api.recipes.update>[1])
  }
  ...
  return (
    <ScrollView style={st.container} contentContainerStyle={st.content}>
      <Text style={st.heading}>Editar Receta</Text>
